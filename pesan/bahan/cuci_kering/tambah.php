...
$query_paket = mysqli_query($koneksi, "SELECT tarif, waktu FROM cuci_kering WHERE id = $paket_id");
if (!$query_paket) {
    die("Query paket gagal: " . mysqli_error($koneksi));
}
$row_paket = mysqli_fetch_assoc($query_paket);
if (!$row_paket) {
    echo "<script>alert('Paket tidak valid!'); window.location.href='tambah.php';</script>";
    exit();
}
...
$query_transaksi = "INSERT INTO tr_cuci_kering 
